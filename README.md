...
**10.As a Pharmacy Owner, I can register my Pharmacy and wait for it to be approved and assured customers that all shops on the platform are legitimate and have a drug license. The Sign Up form lists the personal fields as well as asking for the shop name, DGDA licence number, area, address and contact number (where Register as is Pharmacy Owner). Two rows are written on save; one Users row with UserType 'Admin' and Status 'Pending', and one Pharmacies row with Status 'Pending' that is linked to that Users row. The licence number is registered with a UNIQUE constraint, which means that the licence number can not be issued twice.

**11.. As a Pharmacy Owner, I can edit my shop profile, and edit my password, so that my shop address is always up-to-date, and my account is always secured. The My Pharmacy Profile form updates the Shop name, Area, address, contact phone number and the Logo, and each UPDATE will include WHERE PharmacyId = @PharmacyId, which means that no owner can edit another shop. The licence number is shown only read-only as changing it would result in a new licence and in a new approval.

**12.. As a Pharmacy Owner, I can add a new medicine to my pharmacy, so my customers can discover and purchase my medicine from my pharmacy. A modal dialog with the labels "Medicine" Name, "Generic" Name, "Category" (ComboBox), "Manufacturer", "Strength", "Unit Price", "Stock", "Minimum Stock", "Expiry Date" and a checkbox for "Prescription required" and a text box for the Medicine Description is opened. A unit price must be positive, and a stock and a minimum stock must be 0 or greater, and an expiry date must be in the future, and the name field must not be empty; if any of these rules is not met, a red label will appear under the field and the Save button will remain disabled. When a row is saved, it is saved into Medicines with the logged in owner's PharmacyId, and then it is saved again again using the CHECK constraints in the database and the DataGridView is updated instantly.

**13.As a Pharmacy Owner, I can see which of my medicines are running out, so that I can restock before I lose a sale. The Stock and Inventory form includes four summary tiles and a Low Stock Alert panel. The alert grid uses a query that is filtered by WHERE PharmacyId = @PharmacyId AND Stock < MinStock, and shows the amount of units that need to be ordered. The alert panel is formatted with the background color red on the rows of data by the event CellFormatting. Clicking on a row will activate the Edit Medicine dialog box with the stock field highlighted.

**14. As a Pharmacy Owner, I can read my earnings report, so that I can check what PharmaLink owes me after commission. The Sales and Earnings form takes a date range and an optional medicine filter. It is added to Orders, OrderItems, Medicines and Users to display all of the following information for each line: Order Number, Date and Time, Customer Name, Medicine, Quantity, Unit Price, Subtotal. Four tiles above the grid show gross sales, platform commission, net earnings and units sold. Commission is read from Orders.The price of a medicine cannot change after it has been added to the cart, hence any changes in the medicine price today will not impact the month report.

**15.As a Pharmacy Owner, I can create a discount offer, so that I can move stock during a slow season. The Discount Offers form displays the current offers of the owner, and allows him to add a new offer, by selecting a medicine from a ComboBox, a discount percentage, and a begin date and end date. The percentage must not be less than 0 or higher than 70, and the end date must be after the beginning date; both of these constraints are captured in the form and checked by CHECK constraints on the Offers table. After saving, the discounted price displays automatically on the customers Offers screen for the exact discounts, dates selected.

**16. As a Pharmacy Owner, I can read the reviews written about my medicines, so that I can understand what customers complain about. The Customer Reviews form is a read only form. It displays a DataGridView with a column for the Reviewer Name, Medicine, Rating, Comment, Date and Average rating for medicines of this pharmacy alone. This form never has a Delete button. The owner will use the Report button if he feels that the review is in some way abusive as it will be marked for the Super Admins attention, not for removal by the owner.

**17.As a Pharmacy Owner I can verify a prescription, ensuring that I do not prepare a prescription drug without a doctor's order. If there is a medicine with a "RequiresRx" flag listed on the order, the order displays on the Prescriptions queue with the image uploaded and the doctor's name. The Prescriptions are set when the owner opens the image and clicks on Approve or Reject.VerifyStatus. If the prescription of an order is Pending, then the Confirm button on that order is disabled and a hint is shown explaining why.
